...
class PlayState extends MusicBeatState
{
	...
	public static var typingSongs:Array<String>;
	public static var typingOceanSongs:Array<String>;
	public static var typingScienceSongs:Array<String>;
	public static var typingBlenderSongs:Array<String>;
	...
	var missSounds:FlxTypedGroup<FlxSound> = new FlxTypedGroup<FlxSound>();
	...
	var bfSpelling:FlxText = new FlxText();
	var dadLetters:Map<String, SyllableSound> = new Map<String, SyllableSound>();
	var dadSpelling:FlxText = new FlxText();

	var allSyllableSounds:Array<SyllableSound> = new Array<SyllableSound>();
	var wordsUsed:Map<Int, String> = new Map<Int, String>();
	var wordList:Map<Int, Array<String>> = new Map<Int, Array<String>>();
	...
	override public function create()
	{
		...
		for (i in 1...4)
		{
			missSounds.add(new FlxSound().loadEmbedded('assets/sounds/missnote' + i + TitleState.soundExt)).volume = 0.4;
		}
		add(missSounds);

		var content:String;
		if (FileSystem.exists("assets/data/" + SONG.song.toLowerCase() + "/words.txt"))
			content = sys.io.File.getContent("assets/data/" + SONG.song.toLowerCase() + "/words.txt");
		else
			content = sys.io.File.getContent("assets/data/wordsDefault.txt");
		...
		{
			if (wordList[word.length] == null)
			{
				wordList[word.length] = [word];
			}
			else
			{
		...
		{
			trace(key + " len: " + wordList[key].length);
		}
		...
		typingSongs = ["alphabetic"];
		typingOceanSongs = ["qwerty"];
		typingScienceSongs = ["rollover"];
		typingBlenderSongs = ["tesseract"];
		...
		{
			bfLetters[letter] = new SyllableSound(SONG.player1, letter, false);
			dadLetters[letter] = new SyllableSound(SONG.player2, letter, false);
			allSyllableSounds.push(bfLetters[letter]);
			allSyllableSounds.push(dadLetters[letter]);
		}

		if (FileSystem.exists("assets/data/" + SONG.song.toLowerCase() + "/dialogue-" + SONG.player1 + ".txt"))
		{
			try
			{
				dialogue = CoolUtil.coolTextFile("assets/data/" + SONG.song.toLowerCase() + "/dialogue-" + SONG.player1 + ".txt");
			}
			catch (e)
			{
			}
		...
		if (SONG.stage == null)
		{
			...
			else if (typingSongs.contains(SONG.song.toLowerCase()))
			{
				stageCheck = 'typingStage';
			}
			else if (typingOceanSongs.contains(SONG.song.toLowerCase()))
			{
				stageCheck = 'typingOceanStage';
			}
			else if (typingScienceSongs.contains(SONG.song.toLowerCase()))
			{
				stageCheck = 'typingScienceStage';
			}
			else if (typingBlenderSongs.contains(SONG.song.toLowerCase()))
			{
				stageCheck = 'typingBlenderStage';
			}
		...
		else if (stageCheck == 'typingStage')
		{
			defaultCamZoom = 0.75;
			curStage = 'typingStage';
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image("schoolhouse"));
			bg.antialiasing = true;
			bg.active = false;
			bg.screenCenter(XY);
		...
		else if (stageCheck == 'typingOceanStage')
		{
			defaultCamZoom = 0.75;
			curStage = 'typingOceanStage';
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image("oceanfloor"));
			bg.antialiasing = true;
			bg.active = false;
			bg.screenCenter(XY);
			bg.scrollFactor.set(1, 1);
		...
		else if (stageCheck == 'typingScienceStage')
		{
			defaultCamZoom = 0.75;
			curStage = 'typingScienceStage';
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image("labthing"));
			bg.antialiasing = true;
			bg.active = false;
			bg.screenCenter(XY);
			bg.scrollFactor.set(1, 1);
		...
		else if (stageCheck == 'typingBlenderStage')
		{
			defaultCamZoom = 0.75;
			curStage = 'typingBlenderStage';
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image("blender"));
			bg.antialiasing = true;
			bg.active = false;
			bg.screenCenter(XY);
			bg.scrollFactor.set(1, 1);
		...
		switch (curStage)
		{
			...
			case 'typingStage':
				...
				boyfriend.y += 406;
				gf.x += 150;
			case 'typingOceanStage':
				dad.x -= 175;
				boyfriend.x += 125 + 435;
				boyfriend.y += 406;
				gf.x += 125;
			case 'typingScienceStage':
				dad.x -= 175;
				boyfriend.x += 125 + 435;
				boyfriend.y += 406;
				gf.x += 125;
			case 'typingBlenderStage':
				dad.x -= 150;
				boyfriend.x += 150 + 435;
				boyfriend.y += 406;
				gf.x += 150;
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				// case 'thorns':
				// 	schoolIntro(doof);
				case 'alphabetic' | 'qwerty' | 'rollover':
					someIntro(doof);
				default:
					startCountdown();
			}
		...
		bfSpelling.setFormat(null, 36, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		dadSpelling.setFormat(null, 36, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		bfSpelling.borderSize = dadSpelling.borderSize = 4;
	...
	function someIntro(?dialogueBox:DialogueBox):Void
	{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var notesOrSomething:Array<Note> = [];
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (songNotes[6] != null)
					daWordLength = songNotes[6];
				if (songNotes[7] != null)
					daWordID = songNotes[7];
				...
				swagNote.wordLength = daWordLength;
				swagNote.wordID = daWordID;
				...
				notesOrSomething.push(swagNote);
				...
				if (swagNote.mustPress)
				{
					...
					swagNote.x = 3 * (FlxG.width / 4) - swagNote.width / 2;
				}
				else
				{
					swagNote.x = 1 * (FlxG.width / 4) - swagNote.width / 2;
				}
			}
			daBeats++;
		}

		notesOrSomething.sort(sortByShit);

		for (i in 0...notesOrSomething.length)
		{
			if (notesOrSomething[i].wordLength > -1 && wordsUsed[notesOrSomething[i].wordID] == null)
			{
				if (wordList[notesOrSomething[i].wordLength] == null || wordList[notesOrSomething[i].wordLength].length == 0)
				{
					trace("WORD OF LENGTH " + notesOrSomething[i].wordLength + " NOT AVAILABLE");
					continue;
				}
				var word = FlxG.random.getObject(wordList[notesOrSomething[i].wordLength]);
				trace("NEW WORD: " + word);

				var daMustHit:Bool = notesOrSomething[i].mustPress;

				var j:Int = 0;
				var lettersCopied:Int = 0;
				while (i + j < notesOrSomething.length && lettersCopied < word.length)
				{
					if (notesOrSomething[i + j].mustPress == daMustHit)
					{
						notesOrSomething[i + j].noteLetter = word.charAt(lettersCopied);
						notesOrSomething[i + j].animation.play(word.charAt(lettersCopied));
						lettersCopied++;
					}
					j++;
				}
				wordsUsed[notesOrSomething[i].wordID] = word;
				wordList[notesOrSomething[i].wordLength].remove(word);
			}
			else if (notesOrSomething[i].wordLength > -1)
			{
				var word = wordsUsed[notesOrSomething[i].wordID];
				var daMustHit:Bool = notesOrSomething[i].mustPress;
	...
	private function generateStaticArrows(player:Bool = false):Void
	{
		// for (i in 0...4)
		// {
		// 	// FlxG.log.add(i);
		...
		babyArrow.loadGraphic('assets/images/block.png', true, 154, 154);
		...
		babyArrow.animation.add('static', [0]);
		babyArrow.animation.add('pressed', [1, 2], 12, false);
		babyArrow.animation.add('confirm', [3, 4, 5], 24, false);
		...
		babyArrow.x = (player ? 3 : 1) * (FlxG.width / 4) - babyArrow.width / 2;
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.DELETE && !startingSong)
		{
			health = 0;
			trace("RESET = True");
		}
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (daNote.wordLength > 0)
					{
						addLetter(dad, dadSpelling, daNote.noteLetter, true);
					}
					else if (daNote.noteLetter != null && daNote.noteLetter != "")
					{
						addLetter(dad, dadSpelling, daNote.noteLetter);
					}
				...
				{
					if (boyfriend.canAutoAnim && (!boyfriend.isModel || !daNote.isSustainNote))
					{
						var altString = "";
						switch (Math.abs(daNote.noteData))
						{
				...
				if (daNote.tooLate)
				{
					if (!daNote.didPenalty)
					{
						noteMiss(daNote.noteData, 0.04, false, true);
						muteBF();
						daNote.didPenalty = true;
					}
				}
			...
			notes.forEach(function(daNote:Note)
			{
				...
				{
					handleVocalPlayback(daNote, dadLetters);
				...
				if (!daNote.samplePlayed && daNote.mustPress && daNote.strumTime - Conductor.songPosition <= 0)
				{
					handleVocalPlayback(daNote, bfLetters);
					daNote.samplePlayed = true;
					if (!daNote.wasGoodHit && daNote.wordLength > 0)
						addLetter(boyfriend, bfSpelling, null, true);
				}
			});
		}
	...
	private function keyShit():Void
	{
		...
		var keyHold:Bool = false;
		for (letter in FlxG.keys.getIsDown())
		{
			if (letter.ID.toString().length == 1)
			{
				if (letter.justPressed)
					keyPress = true;
				else
					keyHold = true;
			}
		...
		playerStrums.forEach(function(spr:FlxSprite)
		{
			if (keyPress && spr.animation.curAnim.name != 'confirm')
				spr.animation.play('pressed');
			else if (!keyPress && !keyHold)
	...
	function noteMissWrongPress(direction:Int = 1, ?healthLoss:Float = 0.03):Void
	{
	...
	function badNoteCheck()
	{
		...
		else
		{
			...
			noteMissWrongPress(FlxG.random.int(0, 3), 0.04);
		}
	}
	...
	function missSoundThing()
	{
		for (snd in missSounds.members)
			snd.stop();
		FlxG.random.getObject(missSounds.members).play(true);
	}
	...
	function goodNoteHit(note:Note):Void
	{
		...
		else if (!note.wasGoodHit)
		{
			...
			if (note.noteData >= 0)
			{
				health += 0.04125 * Config.healthMultiplier;
			}
			else
			{
			...
			if (note.wordLength > 0)
			{
				addLetter(boyfriend, bfSpelling, note.noteLetter, true);
			}
			else if (note.noteLetter != null && note.noteLetter != "")
			{
				addLetter(boyfriend, bfSpelling, note.noteLetter);
			}
		}
	}

	function addLetter(person:Character, spellSprite:FlxText, ?letter:String, reset:Bool = false)
	{
		if (reset)
			spellSprite.text = "";
		if (letter != null)
			spellSprite.text += letter;
		spellSprite.x = person.x + person.initWidth / 2 - spellSprite.width / 2;
		spellSprite.y = person.y + person.spellingYOffset;
	}
	...
	function camFocusBF()
	{
		...
		{
			followX = boyfriend.x + boyfriend.initWidth / 2 - 100;
			followY = boyfriend.y + boyfriend.height / 2 - 100;
		}
		else
		{
			followX = boyfriend.getMidpoint().x - 100;
			followY = boyfriend.getMidpoint().y - 100;
		}
