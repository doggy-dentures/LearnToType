...
class Note extends FlxSprite
{
	...
	public var wordLength:Int = -1;
	public var wordID:Int = -1;
	public var noteLetter:String = "";
	...
	public var didPenalty:Bool = false;
	...
	public function new(_strumTime:Float, _noteData:Int, ?_editor = false, ?_prevNote:Note, ?_sustainNote:Bool = false)
	{
		...
		loadGraphic('assets/images/alphabetblocks.png', true, 154, 154);
		setGraphicSize(Std.int(width * 0.7));
		updateHitbox();
		antialiasing = true;
		for (i in 0...Main.alphabet.length)
		{
			animation.add(Main.alphabet[i], [i], 1, false);
		}
		animation.add("error", [26], 1, false);
	...
	override function update(elapsed:Float)
	{
		...
		if (canBeHit && Config.noteGlow && !tooLate && !isSustainNote)
		{
			setColorTransform(1, 1, 1, 1, 128, 128);
		}
		else if (Config.noteGlow && tooLate && !isSustainNote)
		{
			setColorTransform();
		}
